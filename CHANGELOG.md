...
## 2026-08-07 — CI 成本止血：每次推送只跑 Ubuntu，macOS 降为每周（ADR-015，额度耗尽自审）
...
- 修：`ci.yml` 每次推送/PR 只跑 Ubuntu（node 20+22，1x）+ `concurrency: cancel-in-progress` + `paths-ignore` 跳过纯文档提交；新增 `ci-macos.yml` 手动+每周一定时单 job 保留 macOS 覆盖
