...
import { serialize as serializeCollab, TASK_STATES } from './collab/doc.js';
...
async function resolveOrExit(target) {
...
function printMessage(m) {
    ...
    console.log(`[${m.id.slice(0, 8)}] ${m.status}  @${m.from.agent} → @${m.to.agent}:${m.to.sessionId.slice(0, 8)}  ${JSON.stringify(text.length > 120 ? `${text.slice(0, 120)}…` : text)}`);
}
/** Agent labels are opaque doc keys; just guarantee a leading '@'. */
const normalizeLabel = (s) => (s.startsWith('@') ? s : `@${s}`);
...
program
    ...
    if (opts.lan) {
        ...
        lan = {
            ...
            relay: async (device, message) => {
                ...
                if (!peer)
                    return { ok: false, error: `device "${device}" not found on LAN (anyd peers to inspect)` };
    ...
    const directory = async () => {
        ...
        if (lan) {
            ...
            const remotes = await Promise.allSettled(lan.registry.list().map((p) => fetchPeerSessions(p, lan.token)));
    ...
    if (stale > 0)
        console.log(`recovered ${stale} message(s) stranded by a previous daemon run`);
    ...
    const externalNotifier = startExternalInboxNotifier({
        ...
        notifier,
        onNotify: (session, messageId) => console.log(`[${new Date().toISOString()}] external-inbox ${messageId.slice(0, 8)} waiting for @${session.agent}:${session.title} — notified`),
    });
    ...
    const autorun = startAutoRun({
        ...
        onEvent: (e) => {
            console.log(`[${new Date().toISOString()}] autorun ${e.kind} ${e.conversationId.slice(0, 8)}/${e.taskId}${e.detail ? ` — ${e.detail}` : ''}`);
            if (e.kind !== 'error')
                web?.notifyChange();
        },
    });
...
program
    ...
    const opts = {
        ...
        isPullOnly: (sid) => isRegisteredSession(sid),
        onEvent: (e) => console.log(`${e.kind} ${e.message.id.slice(0, 8)}${e.detail ? ` — ${e.detail}` : ''}`),
    };
    let n = 0;
    while (await dispatchOnce(opts))
        n++;
...
program
    ...
    if (opts.json) {
        console.log(JSON.stringify({ sessions: shown, total: sessions.length, errors: errors.map((e) => ({ agent: e.agent, message: e.error.message })) }, null, 2));
        return;
    }
    const { isRegisteredSession } = await import('./registry/external.js');
...
program
    ...
    void (async () => {
        ...
        const { resolveSdsPaths, planSdsInstall, planSdsUninstall, applyChanges } = await import('./integrations/sds.js');
...
program
    ...
    const query = {};
    if (opts.session)
        query.toSession = (await resolveOrExit(opts.session)).sessionId;
    if (opts.take)
        query.take = true;
    if (opts.all)
        query.all = true;
...
program
    ...
    for (;;) {
        for (const t of targets)
            heartbeat(t.id); // tell the daemon: don't resume-deliver to me
        let got = false;
        for (const t of targets) {
            const text = collectInbox(mailbox, t.id);
            if (text) {
...
program
    ...
    for (const c of convos) {
        ...
        console.log(`@${c.a.agent}:${c.a.sessionId.slice(0, 8)} ↔ @${c.b.agent}:${c.b.sessionId.slice(0, 8)}  (${c.messageCount} msgs, ${formatRelativeTime(c.lastMessageAt)})${last}`);
    }
});
...
collab
    ...
    if (opts.conversation) {
        ...
        conversationId = openMailbox().getOrCreateConversation({ agent: self.agent, sessionId: self.sessionId }, { agent: peer.agent, sessionId: peer.sessionId });
    ...
    if (preexisting)
        console.log(`  (doc already existed — left as-is; use anyd collab plan/task to change it)`);
    console.log(`\nNext:`);
    ...
    if (opts.conversation)
        console.log(`  push to the other device:   anyd collab sync ${conversationId} --to @<device>`);
...
collab
    ...
    .action(async (conversationId, opts) => {
    if (!TASK_STATES.includes(opts.state)) {
...
collab
    ...
    .action(async (conversationId, text, opts) => {
    try {
...
collab
    ...
    .action(async (conversationId, newLead, opts) => {
    try {
