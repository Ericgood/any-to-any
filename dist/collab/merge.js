...
function leadWinner(local, incoming) {
    if (incoming.updated > local.updated)
        return incoming;
    if (incoming.updated < local.updated)
        return local;
    const key = (d) => JSON.stringify({ lead: d.lead, tasks: d.tasks, body: d.body });
...
function pickSection(a, b) {
    if (a.entries.length !== b.entries.length)
        return a.entries.length > b.entries.length ? a : b;
    return cmpStr(a.entries.join('\n'), b.entries.join('\n')) >= 0 ? a : b;
}
export function mergeDoc(local, incoming) {
    if (local.conversationId !== incoming.conversationId) {
        throw new Error(`cannot merge different conversations: ${local.conversationId} vs ${incoming.conversationId}`);
    }
    ...
    for (const p of [...local.progress, ...incoming.progress]) {
        if (!agents.includes(p.agent))
            agents.push(p.agent);
    }
    const find = (doc, agent) => doc.progress.find((p) => p.agent === agent);
    const progress = agents.map((agent) => {
