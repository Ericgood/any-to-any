...
const safe = (sessionId) => sessionId.replace(/[^A-Za-z0-9._-]/g, '_');
function monitorDir(home) {
    return join(home, '.anytoany', 'monitors');
}
function monitorPath(sessionId, home) {
...
export function clearMonitor(sessionId, opts = {}) {
    try {
...
export function isMonitored(sessionId, opts = {}) {
    const home = opts.home ?? anytoanyHome();
    const now = opts.now ?? Date.now;
