...
export function parseTarget(raw) {
    ...
    if (!agent)
        return null;
    const out = { agent: agent.toLowerCase() };
    if (device)
        out.device = device.toLowerCase();
    if (fragment)
 ...
 */
export function resolveTarget(raw, sessions) {
    const parsed = parseTarget(raw);
    if (!parsed)
        return { ok: false, reason: 'invalid_target', candidates: [] };
    // no device segment = this machine; '@device/agent' matches that device by ci prefix
    const deviceMatch = (s) => parsed.device
