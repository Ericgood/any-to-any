...
export function defaultAppHome(home = homedir()) {
    return join(home, 'Library', 'Application Support', 'Shandianshuo');
}
export function resolveSdsPaths(opts = {}) {
...
function readWorkspaceOverride(appHome) {
    try {
...
export function sdsSkillMarkdown(port = DEFAULT_PORT) {
    return `---
name: ${SKILL_NAME}
...
function sdsAgentsBlock(port = DEFAULT_PORT) {
...
const change = (path, content, note) => {
...
export function planSdsInstall(paths, opts = {}) {
    ...
    return [
        ...
        change(paths.stateFile, `${JSON.stringify(nextState, null, 2)}\n`, `开启技能开关 "${STATE_KEY}"（手动放入的技能默认是禁用的），其余键原样保留`),
        change(paths.agentsFile, upsertMarkedBlock(readOr(paths.agentsFile), sdsAgentsBlock(port)), '追加常驻指令块（块外内容原样保留）'),
    ];
}
/** Compute the removal of exactly what we installed. */
export function planSdsUninstall(paths) {
    const state = readState(paths.stateFile);
