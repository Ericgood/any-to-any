...
## 2026-09-17 —— 接第三方 App 前先读它的源码，别拿装机产物推断

**错误**：第一版闪电说接入方案是「教助手跑 `anyd send/pull`」，基于装机产物的观察。
**真相**（拉 GitHub 最新分支逐条查源码才发现）：① 助手 shell 继承 App 的最小 GUI PATH
且是非交互 bash，`anyd` 根本摸不到；② 沙箱 `workspace-write` 会拦 `~/.anytoany` 的写入
（但不拦网络）；③ 助手零自驱，收信不可能实时。三条全推翻原方案。
**正确做法**：接任何第三方 App 前，先把它的源码拉下来，逐条确认「环境变量怎么注入、
沙箱边界在哪、有没有自驱能力」，再定通道。装机产物只能看到表象，看不到约束。
**副产物**：约束想清楚后，正解反而更简单——走 `curl` 调本机 daemon，一次绕开全部三条，
而且对用户的权限档位免疫。

## 2026-09-17 —— 单测全绿不等于能跑，真机必须跑一遍

一轮里真机暴露了三个单测抓不到的问题：① commander 命令重名（`setup` 已存在）直接让
整个 CLI 起不来；② 心跳式重复注册不带 title 会把已有 title 冲掉（看日志里的 `@sds:sds`
才发现）；③ daemon 目录缓存 30 秒，**刚注册的 agent 半分钟寻址不到**，第一条回信莫名
`not_found`。
**教训**：daemon / CLI 这类有全局状态和进程边界的改动，测试绿之后必须 build + 重启真
daemon + 端到端跑一遍，并且**给关键路径加日志**——②就是靠日志行发现的，没日志根本看不见。
